...
class BLE_Bleak_Client:
    ...
    async def connect(self, devname, iface, chrc_names, fallback_srv_uuid):
        ...
        if self.devname is None:
            ...
            for d in devices:
                # On macOS, BLEDevice.name is often None until services are
                # discovered; the advertised name lives in AdvertisementData.local_name.
                if d[0].name == self.devname or d[1].local_name == self.devname:
                    found_device = d
